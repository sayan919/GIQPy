...
Flags:
    --traj (Required) : Multi-frame trajectory XYZ file.

    --nFrames (Required) : Number of frames (default: all). For a single frame file, use 1.

    --nDyes (Required) : Number of core monomer units.

    --system_info (Required) : Single JSON defining all monomers and solvent properties.

    --qmSol_radius (Optional) : Defines QM solvent shell by radius (Å) around core atoms.
...
Core Flags:
  ...
  --nDyes <M>                   : Number of core monomer units.
  --system_info <file.json>     : Single JSON defining all monomers and solvent properties.

Optional:
  --qmSol_radius <radius>       : QM solvent shell radius around core atoms.
  --mm_monomer [0|file1 ...]    : MM charges for embedding from other monomers.
  --mm_solvent [file.xyz]       : Defines MM solvent (file or auto-detect).
  --gauss_files [monomer|dimer|both] : Generate Gaussian .com files (requires --gauss_keywords).
  --gauss_keywords <file.txt>   : Gaussian route section keywords.
  --eetg                        : Generate only EETG input for dimers (requires --nDyes=2).
  --tag <TAG_STRING>            : Optional custom tag for filenames.
...
-------------------------------------------------------------------------------
    ...
    parser.add_argument('--nFrames', type=int, default=None,
                        help='Number of frames to process (default: all). Use 1 for a single XYZ file.')
    ...
    parser.add_argument('--eetg', action='store_true',
                        help='Generate only EETG input for dimers (requires --nDyes=2 and --gauss_files).')
    ...
    user_intends_xyz_output = True  # XYZ output is always generated
    ...
    processed_frames_list: List[Tuple[Optional[str], str, str]] = fn.split_frames(None, args.traj, args.nFrames, base_output_dir)
    ...
    try:
        monomers_metadata_list, solvent_metadata = fn.load_system_info(args.system_info, args.nDyes)
    ...
    # .xyz file generation flags (always true for this script)
    actual_gen_monomer_xyz: bool = True
    actual_gen_aggregate_xyz: bool = True
    ...
    for frame_idx, (frame_id_str, temp_xyz_input_filepath, current_frame_output_dir) in enumerate(processed_frames_list):
        ...
        try:
            ...
            core_coords_for_dist_calc, n_atoms_per_monomer_for_dist_calc = \
                fn.localize_solvent_and_prepare_regions(
                    ...
                    args.nDyes,
                    args.qmSol_radius,
                    qm_aggregate_coords_override=None
                )
        ...
        if args.mm_monomer is not None: 
            if args.mm_monomer == ['0']: 
                ...
                if len(args.mm_monomer) == args.nDyes :
                    fn.write_to_log(f"Loading MM charges for inter-monomer embedding from {args.nDyes} specified file(s).")
        ...
        if args.traj and os.path.exists(temp_xyz_input_filepath) and fn.TEMP_FRAME_XYZ_FILENAME in temp_xyz_input_filepath :
            try:
